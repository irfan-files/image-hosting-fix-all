      ...
      <div key={folder.id} className="select-none">
        ...
        >
          <div className="flex items-center gap-2 overflow-hidden">
            ...
            <FolderIcon
              className={`w-4 h-4 shrink-0 ${isSelected ? "text-indigo-600 dark:text-indigo-400" : "text-amber-500"}`}
            />

            {editingFolderId === folder.id ? (
              <form
                onSubmit={(e) => handleRenameSubmit(folder.id, e)}
                className="flex-1"
          ...
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button
              onClick={(e) => {
                ...
                if (
                  confirm(
                    `Delete folder "${folder.name}" and move items to trash?`,
                  )
                ) {
        ...
        <div>
          ...
          <div className="space-y-0.5">
            {rootFolders.length === 0 ? (
              <p className="text-xs text-slate-400 dark:text-slate-500 italic px-2.5 py-1">
