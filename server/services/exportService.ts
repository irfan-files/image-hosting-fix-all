...
export class ExportService {
  ...
  public static generateCSV(
    ...
    const rows = sorted.map((img) => {
      ...
      if (columns.originalSize)
        row["Original Size"] = this.formatBytes(img.originalSize);
      if (columns.compressedSize)
        row["Compressed Size"] = this.formatBytes(img.compressedSize);
      ...
      if (columns.uploadDate)
        row["Upload Date"] = new Date(img.createdAt).toLocaleString();
   ...
   */
  public static async generateXLSX(
    images: ImageItem[],
    columns: ExportColumnOptions,
    sheetTitle?: string,
  ): Promise<Buffer> {
    ...
    // Excel worksheet names must be <= 31 chars and no invalid characters: \ / ? * : [ ]
    let safeSheetName = (sheetTitle || "Image Catalog")
      .replace(/[\\/?*\[\]:]/g, "_")
      .trim()
      .slice(0, 31);
    if (!safeSheetName) safeSheetName = "Image Catalog";
    ...
    if (columns.folder)
      colHeaders.push({ header: "Folder Path", key: "folderPath", width: 30 });
    if (columns.filename)
      colHeaders.push({
        header: "Filename",
    ...
    if (columns.originalSize)
      colHeaders.push({
        ...
        key: "originalSizeFormatted",
        width: 15,
      });
    if (columns.compressedSize)
      colHeaders.push({
        header: "Compressed Size",
        key: "compressedSizeFormatted",
        width: 15,
      });
    if (columns.width)
      colHeaders.push({ header: "Width (px)", key: "width", width: 12 });
    if (columns.height)
      colHeaders.push({ header: "Height (px)", key: "height", width: 12 });
    if (columns.format)
      colHeaders.push({ header: "Format", key: "extension", width: 12 });
    if (columns.uploadDate)
      colHeaders.push({
        header: "Upload Date",
    ...
    sorted.forEach((img) => {
      ...
      if (columns.originalSize)
        rowData["originalSizeFormatted"] = this.formatBytes(img.originalSize);
      if (columns.compressedSize)
        rowData["compressedSizeFormatted"] = this.formatBytes(
          img.compressedSize,
        );
      ...
      if (columns.uploadDate)
        rowData["createdAtFormatted"] = new Date(
          img.createdAt,
        ).toLocaleString();
   ...
   */
  public static async generateMarketplaceMatrixXLSX(
    images: ImageItem[],
    sheetTitle?: string,
  ): Promise<Buffer> {
    const collator = new Intl.Collator(undefined, {
    ...
    let safeSheetName = (sheetTitle || "Marketplace Template")
      .replace(/[\\/?*\[\]:]/g, "_")
      .trim()
      .slice(0, 31);
    if (!safeSheetName) safeSheetName = "Marketplace Template";
